...
public class BST<T extends Comparable<T>> extends BinaryTree<T> {
    ...
    public String getPathToLeafNode(T e) {
        ...
        String path = getPathToLeafNode(root, e);
        if (path == null) {
            throw new IllegalArgumentException("Node with data " + e + " does not exist in the tree");
        }
        return path;
    }

    // Recursively finds the path from the root to a leaf node with data 'e'.
    private String getPathToLeafNode(BTNode<T> node, T e) {
        if (node == null) {
            return null;
        }
        if (node.data.equals(e) && node.left == null && node.right == null) {
            return node.data.toString();
        }
        String Pathleft = getPathToLeafNode(node.left, e);
        if (Pathleft != null) {
            return node.data + " " + Pathleft;
        }
        String Pathright = getPathToLeafNode(node.right, e);
        if (Pathright != null) {
            return node.data + " " + Pathright;
        }
        return null;
    }
    ...
    public int getNodeLevel(T e) {
        ...
        int level = getNodeLevel(root, e, 1);
        if (level == -1) {
            throw new NoSuchElementException("No node with data " + e + " in the tree");
        }
        return level;
    }

    // Recursively determines the level of a node with data 'e' in the tree.
    private int getNodeLevel(BTNode<T> node, T e, int currentLevel) {
        if (node == null) {
            return -1;
        }
        ...
        int leftLevel = getNodeLevel(node.left, e, currentLevel + 1);
        if (leftLevel != -1) {
            return leftLevel;
        }
        return getNodeLevel(node.right, e, currentLevel + 1);
    }
}
